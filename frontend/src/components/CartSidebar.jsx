            ...
            <div className="flex flex-col gap-4">
              {cart.map((item) => (
                <div key={item.id} className="flex items-center gap-4 py-4 border-b border-[#2a2d35]">
                  <div className="w-12 h-12 bg-[#16181c] flex items-center justify-center text-2xl flex-shrink-0">
                    {item.emoji}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-['Barlow_Condensed'] font-bold text-sm uppercase text-white truncate">
                      {item.brand} {item.name}
                    </div>
                    <div className="text-[#e8f216] text-sm font-semibold mt-0.5">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className="w-7 h-7 border border-[#2a2d35] text-[#6b7280] hover:border-[#e8f216] hover:text-[#e8f216] transition-all flex items-center justify-center text-lg leading-none"
                    >−</button>
        ...
        {cart.length > 0 && (
          <div className="px-6 py-5 border-t border-[#2a2d35]">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="font-['Barlow_Condensed'] font-black text-2xl text-white">
                {subtotal.toLocaleString('ru')} ₽
              </span>
            </div>
            <button
              onClick={onCheckout}
